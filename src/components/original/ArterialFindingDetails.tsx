...
import { Ruler, MapPin, Activity, Plus, X, Save, Zap, AlertTriangle } from 'lucide-react';

interface ArterialFindingDetailsProps {
  finding: Finding;
  organId: string;
  severity?: string;
...
const ArterialFindingDetails: React.FC<ArterialFindingDetailsProps> = ({
  finding,
  organId,
  severity,
  instances = [],
  ...
  const getSeverityBadgeColor = (sev?: string): string => {
    ...
    if (lower.includes('grave') || lower.includes('>75') || lower.includes('crítica') || lower.includes('aguda')) return 'bg-red-600';
    if (lower.includes('moderada') || lower.includes('50-75')) return 'bg-orange-500';
    if (lower.includes('leve') || lower.includes('<50')) return 'bg-yellow-500';
    return 'bg-blue-500';
  };
  ...
  return (
    <div className="space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div key={meas.id} className="space-y-1">
                  ...
                  <div className="flex gap-2">
                    <Input
                      type="number"
                      step="0.1"
                      placeholder={`Ex: ${meas.id === 'psv' ? '120' : '2.5'}`}
      ...
      {(finding.id.includes('oclusao') || finding.id.includes('grave')) && (
        <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-md flex items-start gap-2">
          ...
          <p className="text-xs text-yellow-800">
            <strong>Atenção:</strong> {finding.id.includes('oclusao')
              ? 'Oclusão arterial requer avaliação vascular urgente. Considere classificação de Rutherford e isquemia aguda.'
              : 'Estenose grave (>75%) pode requerer intervenção. Avalie sintomatologia clínica.'}
          </p>
        </div>
      )}
    </div>
...
export default ArterialFindingDetails;
