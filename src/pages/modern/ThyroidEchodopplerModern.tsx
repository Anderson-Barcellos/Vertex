/**
 * Vertex V2 - Thyroid Echodoppler Exam (MODERN DESIGN)
 * Layout moderno compartilhado baseado no Abdome Modern / Carotid Modern
 */
...
function ThyroidEchodopplerModern() {
  ...
  const handleGenerateReport = async (
    ...
    try {
      ...
      const promptText = buildSpecializedPrompt({
        examType: 'Ultrassonografia com Ecodoppler de Tireóide',
              ...
              onError: async (error) => {
                ...
                try {
                  const fallback = await generateReport(
                    ...
                    { organsList: thyroidOrgans, provider }
                  );
  ...
  const currentOrgan = thyroidOrgans.find(organ => organ.id === selectedOrgan);
        ...
        header={(
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-white">Ecodoppler de Tireóide</h1>
                <p className="text-xs text-gray-400">Ultrassonografia com Doppler + TI-RADS</p>
              </div>
            </div>
          </div>
        )}
...
export default ThyroidEchodopplerModern;
