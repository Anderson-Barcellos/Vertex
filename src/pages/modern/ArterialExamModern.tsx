/**
 * Vertex V2 - Arterial Exam (MODERN DESIGN)
...
function ArterialExamModern() {
              ...
              onError: async (error) => {
                ...
                try {
                  const fallback = await generateReport(
                    ...
                    { organsList: arterialOrgans, provider }
                  );
  ...
  const currentOrgan = arterialOrgans.find(organ => organ.id === selectedOrgan);
  ...
  const currentOrganIndex = arterialOrgans.findIndex(organ => organ.id === selectedOrgan);
  const hasPreviousOrgan = currentOrganIndex > 0;
  const hasNextOrgan = currentOrganIndex < arterialOrgans.length - 1 && currentOrganIndex >= 0;

  const handlePreviousOrgan = useCallback(() => {
    if (hasPreviousOrgan) {
      const prevOrgan = arterialOrgans[currentOrganIndex - 1];
  ...
  const handleNextOrgan = useCallback(() => {
    if (hasNextOrgan) {
      const nextOrgan = arterialOrgans[currentOrganIndex + 1];
        ...
        header={(
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-white">Ecodoppler Arterial</h1>
        ...
        floatingPanel={(
          currentOrgan ? (
            <FloatingOrganPanelModern
              ...
              totalOrgans={arterialOrgans.length}
            />
          ) : null
        )}
      />
...
export default ArterialExamModern;
